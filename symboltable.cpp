...
void clearOutput() {
    ...
    if (!output.is_open()) {
        cerr << "Error deleting file contents.\n";
    }
    output.close();
}
...
public:
    ...
    void insert(Symbol_Info *newT) {
        ...
        if (!snf[id]) {
            ...
            cout << "Inserted at position " + to_string(id) + ", 0\n";
        } else {
            ...
            newS->prev = tmp;
            tmp->next = newS;
            cout << "Inserted at position " + to_string(id) + ", " + to_string(the_ind) + "\n";
        }
    }
    ...
    Symbol_Info *lookUp(string si, bool c = true) {
        ...
        if (snf[id]) {
            if (snf[id]->getName() == si) {
                cout << ((c) ? "Found at " : "Deleted from ") + to_string(id) + ", 0\n";
                return snf[id];
            } else {
                ...
                while (tmp) {
                    if (tmp->getName() == si) {
                        cout << ((c) ? "Found at " : "Delete from ") + to_string(id) + "," + to_string(newIndex) + "\n";
                        return tmp;
                    }
                    tmp = tmp->next;
                    newIndex++;
...
int main() {
    ...
    while (input >> c) {
        if (c == 'I') {
            ...
            mySymbolTable.insert(new Symbol_Info(n, t));
        } else if (c == 'P') {
            mySymbolTable.print();
        } else if (c == 'L') {
            string s; input >> s;
            mySymbolTable.lookUp(s);
        } else if (c == 'D') {
            string s; input >> s;
            mySymbolTable.del(s);
        } else {
